  ...
  Compute(sparta, narg, arg)
{
  ...
  npergroup = 6+1; // ond additional tally for sum of weights
                  ...
                  double **etally, int *emap, double *vec, int nstride)
{
  ...
  Grid::ChildInfo *cinfo = grid->cinfo;
  double ncount,mass,mvx,mvy,mvz,mvsq,sumg;
  double *values;
  ...
  for (int icell = lo; icell < hi; icell++) {
    ...
    if (ncount <= 1.0) vec[k] = 0.0;
      ...
      else if (value[ivalue] == PRESS) prefactor = pprefactor*gavg;
