...
enum{ID,TYPE,PROC,X,Y,Z,XS,YS,ZS,VX,VY,VZ,SW,KE,EROT,EVIB,
  ...
  if (nthresh) {
    ...
    for (int ithresh = 0; ithresh < nthresh; ithresh++) {
      ...
      if (thresh_array[ithresh] == ID) {
        ...
        ptr = dchoose;
        nstride = 1;
      } else if (thresh_array[ithresh] == SW) {
        for (i = 0; i < nlocal; i++) dchoose[i] = particles[i].sw;
        ptr = dchoose;
        nstride = 1;
...
int DumpParticle::parse_fields(int narg, char **arg)
{
  ...
  for (int iarg = 0; iarg < narg; iarg++) {
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      ...
      pack_choice[i] = &DumpParticle::pack_sw;
      vtype[i] = DOUBLE;
...
int DumpParticle::modify_param(int narg, char **arg)
{
  ...
  if (strcmp(arg[0],"thresh") == 0) {
    ...
    else if (strcmp(arg[1],"sw") == 0) thresh_array[nthresh] = SW;
...
void DumpParticle::pack_sw(int n)
{
  Particle::OnePart *particles = particle->particles;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = particles[clist[i]].sw;
    n += size_one;
  }
}
