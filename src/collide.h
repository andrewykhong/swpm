...
class Collide : protected Pointers {
 public:
  ...
  virtual double attempt_collision_sum(int, int, double, double) = 0;
  virtual double attempt_collision_max(int, int, double) = 0;
  virtual int test_collision(int, int, int,
                             Particle::OnePart *, Particle::OnePart *) = 0;
  virtual int test_collision_sum(int, int, int,
                             Particle::OnePart *, Particle::OnePart *) = 0;
  virtual int test_collision_max(int, int, int,
                             Particle::OnePart *, Particle::OnePart *) = 0;
  ...
  virtual int perform_collision(Particle::OnePart *&, Particle::OnePart *&,
                                Particle::OnePart *&) = 0;
  virtual void perform_collision_sw(Particle::OnePart *&, Particle::OnePart *&,
                                Particle::OnePart *&) = 0;
  virtual void perform_collision_sw(Particle::OnePart *&, Particle::OnePart *&) = 0;
  virtual void perform_split(Particle::OnePart *&, Particle::OnePart *&,
                        Particle::OnePart *&, Particle::OnePart *&) = 0;
 ...
 protected:
  ...
  int Nmax;             // Particles combine if local count > Nmax
  //double **gmax;        // maximum weight (constant)
  //double gmax_initial;  // intial
  double gmn;           // minimum weight (variable)
  double gmx;           // maximum weight (variable)
  double Ggamma;
  int gamFlag;

  // used for grouping and merging
  double rhoth;           // threshold temperature for halting division
  double **Rij;           // covariance matrix
  int *currentCluster;    // cluster to be merged next
  int maxCluster;         // max size of currentCluster
  int npCluster;          // number of particles in cluster
  ...
  int reduction_type;     // indicates which reduction to use
  double iT;              // temperature
  double igsum;           // sum of weights
  double *ie;             // energy components
  double *iV;             // center of mass velocity
  double *iq;             // heat flux vector
  double **ipij;          // stress tensor

  // subcells to separate particles in space
  // preserve internal gradients
  //int subNx, subNy, subNz, subN;
  //int **subCell;
  //int *subCellN;

  // for LGZ merge and grouping
  // probably can make these local
  double *eval;           // eigenvalues
  double **evec;          // eigenvectors

  // for GONO merge
  int x2flag;
  int v2flag;
  int v3flag;
  int nGono; // number of particles to reduce to
  int nm; // number of constraints

  // flag for different position schemes
  double *xmr;
  int rFlag;

  // Debug tools. Always comment out when not using
  double debugD0;
  double debugD1;
  double debugD2;
  ...
  template < int > void collisions_one_sw();
