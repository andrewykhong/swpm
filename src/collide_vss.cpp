...
double CollideVSS::vremax_init(int igroup, int jgroup)
{
  ...
  double vrmgroup = 0.0;
  //printf("vre_init. igroup: %i, jgroup: %i, vrmgroup: %f", vrmgroup);
...
double CollideVSS::attempt_collision(int icell, int np, double volume)
{
  ...
  if(nattempt>MAXSMALLINT) error->one(FLERR,"Bad attempts calculated");
  return nattempt;
}
...
double CollideVSS::attempt_collision_sum(int icell, int np, double volume, double gsum)
{
  double dt = update->dt;
  ...
  double gavg = gsum/np;
  if(gamFlag > 0) nattempt = 0.5 * np * (np-1) * (1.0 + Ggamma) *
    vremax[icell][0][0] * dt * (2.0*gavg) / volume + random->uniform();
  else nattempt = 0.5 * np * (np-1) *
    vremax[icell][0][0] * dt * (2.0*gavg) / volume + random->uniform();

  if(nattempt>MAXSMALLINT) {
    printf("np: %i; gavg: %4.3e; gmn: %4.3e\n", np, gavg, gmn);
    printf("Vre: %4.3e; volume: %4.3e\n", vremax[icell][0][0], volume);
    printf("nattempt: %4.3e\n", nattempt);
    error->one(FLERR,"Bad attempts calculated");
  }
...
double CollideVSS::attempt_collision_max(int icell, int np, double volume)
{
  double dt = update->dt;
  ...
  if(gamFlag > 0) nattempt = 0.5 * np * (np-1) * (1.0 + Ggamma) *
    vremax[icell][0][0] * dt * (gmx) / volume + random->uniform();
  else nattempt = 0.5 * np * (np-1) *
    vremax[icell][0][0] * dt * (gmx) / volume + random->uniform();

  if(nattempt>MAXSMALLINT) {
    printf("np: %i; gmx: %4.3e\n", np, gmx);
    printf("Vre: %4.3e; volume: %4.3e\n", vremax[icell][0][0], volume);
    printf("nattempt: %4.3e\n", nattempt);
    error->one(FLERR,"Bad attempts calculated");
  }
                               ...
                               Particle::OnePart *ip, Particle::OnePart *jp)
{
  ...
  if(vre<0) error->one(FLERR,"Negative vre");
  if(vre>MAXSMALLINT) error->one(FLERR,"vre near inf");
...
/* ----------------------------------------------------------------------
   determine if sw collision actually occurs
   1 = yes, 0 = no
   update vremax either way
------------------------------------------------------------------------- */

int CollideVSS::test_collision_sum(int icell, int igroup, int jgroup,
                               Particle::OnePart *ip, Particle::OnePart *jp)
{
  double *vi = ip->v;
  double *vj = jp->v;
  ...
  // use sum of individual
  double gp = MAX(gi,gj)/(gi+gj);
  if(gp > 1) error->one(FLERR,"probabiltiy > 1");

  vremax[icell][igroup][jgroup] = MAX(vre,vremax[icell][igroup][jgroup]);
  if (vre/vremax[icell][igroup][jgroup]*gp < random->uniform()) return 0;
  precoln.vr2 = vr2;
  return 1;
}
...
int CollideVSS::test_collision_max(int icell, int igroup, int jgroup,
                               Particle::OnePart *ip, Particle::OnePart *jp)
{
  double *vi = ip->v;
  double *vj = jp->v;
...
void CollideVSS::perform_collision_sw(Particle::OnePart *&ip,
                                      Particle::OnePart *&jp,
                                      Particle::OnePart *&kp)
{
  double gi = ip->sw;
  ...
  kp = NULL;
  // same weight
  if(fabs(1.0-gi/gj)<SMALLNUM) {
    setup_collision(ip,jp);
    SCATTER_TwoBodyScattering(ip,jp);
    return;
  } else if(gi>gj) {
    gk = gi-gj;
    ...
    int reallocflag =
      particle->add_particle(id,ip->ispecies,ip->icell,x,v,0.0,0.0,gk);
    if (reallocflag) {
      ip = particle->particles + (ip - particles);
      jp = particle->particles + (jp - particles);
    }
    ...
    int reallocflag =
      particle->add_particle(id,jp->ispecies,jp->icell,x,v,0.0,0.0,gk);
    if (reallocflag) {
      ip = particle->particles + (ip - particles);
      jp = particle->particles + (jp - particles);
    }
...
void CollideVSS::perform_split(Particle::OnePart *&ip,
                               Particle::OnePart *&jp,
                               Particle::OnePart *&kp,
                               Particle::OnePart *&lp)
{
  ...
  double x[3],v[3];
  // weight transfer
  // Assume G = min(gi,g)/(1+Ggamma)
  double gi = ip->sw;
  double gj = jp->sw;

  // checks if particles properly deleted
  if(gi<0 || gj<0) error->one(FLERR,"negative weight");
  int id;
  int reallocflag;
  Particle::OnePart *particles = particle->particles;
  ...
  if(Ggamma>0) {
    ...
    memcpy(x,ip->x,3*sizeof(double));
    memcpy(v,ip->v,3*sizeof(double));
    reallocflag = particle->add_particle(id,ip->ispecies,ip->icell,x,v,0.0,0.0,G);
    if (reallocflag) {
      ip = particle->particles + (ip - particles);
      jp = particle->particles + (jp - particles);
    }
    ...
    memcpy(x,jp->x,3*sizeof(double));
    memcpy(v,jp->v,3*sizeof(double));
    reallocflag = particle->add_particle(id,jp->ispecies,jp->icell,x,v,0.0,0.0,G);
    if (reallocflag) {
      ip = particle->particles + (ip - particles);
      jp = particle->particles + (jp - particles);
