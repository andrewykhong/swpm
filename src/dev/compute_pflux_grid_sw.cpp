...
#include "string.h"
#include "compute_pflux_grid_sw.h"
#include "particle.h"
#include "mixture.h"
#include "grid.h"
#include "update.h"
...
enum{MASSSUM,MVXSQ,MVYSQ,MVZSQ,MVXVY,MVYVZ,MVXVZ,LASTSIZE};
...
ComputePFluxGridSW::ComputePFluxGridSW(SPARTA *sparta, int narg, char **arg) :
...
ComputePFluxGridSW::~ComputePFluxGridSW()
{
  if (copymode) return;
...
void ComputePFluxGridSW::compute_per_grid()
{
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    for (m = 0; m < npergroup; m++) {
      switch (unique[m]) {
      case MASSSUM: // sum of weights
        vec[k++] += g;
        break;
      case MVXSQ:
        vec[k++] += mass*g*gsum/(g+gsum)*dx*dx;
        break;
      case MVYSQ:
        vec[k++] += mass*g*gsum/(g+gsum)*dy*dy;
        break;
      case MVZSQ:
        vec[k++] += mass*g*gsum/(g+gsum)*dz*dz;
        break;
      case MVXVY:
        vec[k++] += mass*g*gsum/(g+gsum)*dx*dy;
        break;
      case MVYVZ:
        vec[k++] += mass*g*gsum/(g+gsum)*dy*dz;
        break;
      case MVXVZ:
        vec[k++] += mass*g*gsum/(g+gsum)*dx*dz;
        break;
      }
    }
    gsum += g;
...
int ComputePFluxGridSW::query_tally_grid(int index, double **&array, int *&cols)
{
  index--;
...
void ComputePFluxGridSW::post_process_grid(int index, int nsample,
...
void ComputePFluxGridSW::set_map(int ivalue, int name)
{
...
void ComputePFluxGridSW::reset_map()
{
...
void ComputePFluxGridSW::reallocate()
{
...
bigint ComputePFluxGridSW::memory_usage()
{
  bigint bytes;
