...
enum{MASSSUM,mVx,mVy,mVz,
     mCxCsq, mCyCsq, mCzCsq,
     LASTSIZE};
...
void ComputeEFluxGridSW::compute_per_grid()
{
  ...
  // calculate particular velocity and heat fluxs
  double gmass, Ux, Uy, Uz;
  double csq, c[3];
  for (i = 0; i < nlocal; i++) {
    ispecies = particles[i].ispecies;
    igroup = s2g[ispecies];
                                         ...
                                         double *vec, int nstride)
{
  ...
  for (int icell = lo; icell < hi; icell++) {
    ...
    vec[k] = wt/nsample * t[h];
    k += nstride;
  }
  //error->one(FLERR, "check k sw");
}
